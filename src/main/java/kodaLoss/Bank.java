...
public class Bank {
	...
	/*
	 * Reference to UserChoiceAdapter just because its easier to write 'uca'
	 * than 'UserChoiceAdapter.getInstance().xxx' all the time
	 */
	...
	/*
	 * reference to the Player objects of dealer and the active player to be
	 * shown on the GUI. Needed for updating the GUI.
	 */
	 ...
	 * directly calling methods in Controller. Bank decides this way when GUI
	 * should be changed
	 */
	...
	/**
	 * This method controls the sequence of actions to play one round, control
	 * of the gameplay! Has to run in its own thread because method blocks while
	...
	public void askPlayersForBetsForThisRound() {
		...
		for (Player p : registeredPlayers) {
			...
			controller.setlabelWinnerText(p.getName() + ", " + BlackJackConstantsAndTools.ASK_FOR_BETS);
	...
	public void calculateWinners() {
		 ...
		 * => WIN = CASE 1 DEALER BUST -> Player bust => Loose = CASE 2 Dealer
		 * BJ -> Player BJ => TIE = CASE 3 DEALER BJ -> Player <= 21 => Loose =
		 * CASE 4 Dealer <= 21 -> Player Bust => Loose = CASE 5 Dealer <= 21 ->
		 * Player < dealer => Loose = CASE 6 DEALER <= 21 -> Player > dealer =>
		 * WIN = CASE 7 DEALER <= 21 -> Player = dealer => TIE = CASE 8 Dealer
		 * <=21 -> Player = BJ => WIN = case 9
		 */
	...
	public void roundOutcome() {
		...
		for (Player p : registeredPlayers) {
			...
			} else if (calculateValueOfPlayersHand(p) == calculateValueOfPlayersHand(dealer)) {
				System.out.println("It´s a TIE.");
				winnerText = BlackJackConstantsAndTools.RESULT_A_TIE;
				p.setRoundResult(RoundResult.TIE);
			}
			// player not Bust - CASE 1
			else if (calculateValueOfPlayersHand(p) <= 21 && calculateValueOfPlayersHand(dealer) <= 21) {
				if (calculateValueOfPlayersHand(p) > calculateValueOfPlayersHand(dealer)) {
					System.out.println("Congratulations! You won.");
					winnerText = BlackJackConstantsAndTools.RESULT_YOU_WON;
					p.setRoundResult(RoundResult.WIN);
				} else if (calculateValueOfPlayersHand(p) < calculateValueOfPlayersHand(dealer)) {
	...
	/*
	 * uses players bets and result of played round to calculate money to be
	 * payed back to players. Loosing players loose their bet, Tie players get
	 * their bet back, winning players get the double amount of their bet,
	 * winning players with a Blackjack even get an extra 50% of their bet!
	 */
	public void handlePlayersBetsAndPayWinners() {

		/*
		 * TIE => return betting amount WIN => return 2 * bet WIN + BJ => return
		 * 2.5 bet Loose => players bet is not returned
		 */
		int playersBet;
