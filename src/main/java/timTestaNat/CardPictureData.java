...
import static kodaLoss.BlackJackConstantsAndTools.FILE_PICTURE_FILEENDING;
import static kodaLoss.BlackJackConstantsAndTools.FILE_PICTURE_PATH;
    ...
    for (Suite s : Suite.values()) {
      for (Rank r : Rank.values()) {
        ...
        try {
          ...
          imageData[s.ordinal()][r.ordinal()] = new Image(FILE_PICTURE_PATH + s.toString() + " "
              + r.toString() + FILE_PICTURE_FILEENDING , 75 , 150 , true , true);//ImageIO.read(imgFile);
        } catch (Exception e) {
